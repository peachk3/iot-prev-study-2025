...
class Contact : # 클래스를 정의
    # 클래스에 속하는 함수들은 인자(매개변수)의 첫 번째에 self를 반드시 작성해야 함!!
    ...
    # 12. 이름이 존재하는 객체 확인 함수
    def isNameExist(self, name): # 클래스 하위의 함수 /self -> 클래스에 포함되어 있는 함수라는 의미
        if self.__name == name:
